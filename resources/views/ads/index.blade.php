<x-app-layout>
   ...
   <div style="display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: 1fr; grid-column-gap: 1em; grid-row-gap: 2em;">
      @foreach ( $ads as $ad)
      <a href="{{ route('ads.get', ['id' => $ad->id]) }}" style="text-decoration: none; color: inherit;">
         <article style="margin-bottom: 0; padding-top: 1em; padding-bottom: 1em; height: 18em;">
            <p><b> {{ Str::of($ad->title)->limit(20) }}</b></p>
            <p>
		...
		<image src="https://picsum.photos/800/600"></image>
            ...
            <p>
		<span style="color: #424751;"><i>since {{ $ad->created_at->isSameDay(now()) ? $ad->created_at->format('H:i') : $ad->created_at->format('Y m d') }}</i></span>
      </i></span> 
            </p>
            <p>
               <span>
